...
def chk_usb():
    ...
    if(mounted == "None"):      # usb is inserted and not mounted
        ...
        loc1 = childrn_str.find("sd")  # find sda1 or sda2
...
#            os.system("mkdir -p /media/pi/mount")
#            os.system("sudo mount /dev/sda1 /media/pi/mount")
...
#                     os.system(cmd)
        ...
        if (usbinserted == 2):
            ...
            RTCTime = getRTCtime()
            data_file_name = newname + "_" + RTCTime[0:10]
            data_temp_zip_file = opj(mydir, data_file_name + ".zip")
            data_temp_txt_file = opj(mydir, data_file_name + ".txt")
            shutil.copyfile(opj(mydir, "Electroguard.txt"), data_temp_txt_file)
            with zipfile.ZipFile(data_temp_zip_file, "w", compression=zipfile.ZIP_DEFLATED) as newzip:
                newzip.write(data_temp_txt_file, arcname=data_temp_txt_file.split('/')[-1])
            for d in list_dirs():
                print("Copying {} to {}".format(data_temp_zip_file, opj(udisks_path, d.name, data_file_name + ".zip")))
                shutil.copyfile(data_temp_zip_file, opj(udisks_path, d.name, data_file_name + ".zip"))

            # clean up Electroguard.txt NewFile
            os.remove(data_temp_txt_file)
            os.remove(data_temp_zip_file)

            # second the small data file
            diag_file = opj(mydir, 'diags.txt')
            if exists(diag_file):
                diag_file_name = str(sernum) + "diags" + "_" + RTCTime[0:10]
                diag_zip_temp_file = opj(mydir, diag_file_name + ".zip")
                diag_text_temp_file = opj(mydir, diag_file_name + ".txt")
                shutil.copyfile(diag_file, diag_text_temp_file)
                with zipfile.ZipFile(diag_zip_temp_file, "w", compression=zipfile.ZIP_DEFLATED) as newzip:
                    newzip.write(diag_text_temp_file, arcname=diag_text_temp_file.split('/')[-1])
                for d in list_dirs():
                    print("Copying {} to {}".format(diag_zip_temp_file, opj(udisks_path, d.name, diag_file_name + ".zip")))
                    shutil.copyfile(diag_zip_temp_file, opj(udisks_path, d.name, diag_file_name + ".zip"))

                # clean up NewFile small data file
                os.remove(diag_text_temp_file)
                os.remove(diag_zip_temp_file)

            # finally umount drives
            for d in list_dirs():
                os.sync()
                print("Unmounting {}".format(opj(udisks_path, d.name)))
                subprocess.check_call(["/usr/bin/umount", opj(udisks_path, d.name)])
...
def get_adcs():
    ...
    time.sleep(0.001)
    CH0 = ((float)((((((CH0_raw[1] & 0x0F) << 8) + CH0_raw[2]) - 1365) * 7.5 / 4096)) + (ch1offset - 128)/1000)
    CH0 = CH0 * (ch1gain / 128)

    if((chansel & 0x01) != 0x01):
        ButtCol1 = "#A6ACAF"
    else:
        if (CH0 < RedYell):
            ButtCol1 = "red"
...
# conversion of CH1
    ...
    CH1 = ((float)((((((CH1_raw[1] & 0x0F)<<8) + CH1_raw[2]) - 1365) * 7.5 / 4096)) + (ch2offset - 128)/1000)
    # CH1 = ((float)((((CH1_raw[1] & 0x0F)<<8) + CH1_raw[2] - 1365) * 7.5 / 4096))
    CH1 = CH1 * ch2gain/128
    GPIO.output(ADC_CS,GPIO.HIGH)
    if((chansel & 0x02) != 0x02):
        ButtCol1 = "#A6ACAF"
    else:
        if (CH1 < RedYell):
            ButtCol2 = "red"
...
# conversion of CH2
    ...
    GPIO.output(ADC_CS,GPIO.HIGH)
    CH2 = ((float)((((((CH2_raw[1] & 0x0F)<<8) + CH2_raw[2]) - 1365) * 7.5 / 4096)) + (ch3offset - 128)/1000)
    CH2 = CH2 * (ch3gain / 128)
    if((chansel & 0x04) != 0x04):
        ButtCol1 = "#A6ACAF"
    else:
        if (CH2 < RedYell):
            ButtCol3 = "red"
...
# conversion of CH3
    ...
    GPIO.output(ADC_CS,GPIO.HIGH)
    CH3 = ((float)((((((CH3_raw[1] & 0x0F)<<8) + CH3_raw[2]) - 1365) * 7.5 / 4096)) + (ch4offset - 128)/1000)
    CH3 = CH3 * (ch4gain / 128)
    if((chansel & 0x08) != 0x08):
        ButtCol1 = "#A6ACAF"
    else:
        if (CH3 < RedYell):
            ButtCol4 = "red"
...
#    print("make sure p1 exists, every sec")
...
#        print (RTCTime)
        ...
        if ((chansel & 0x0F) == 0x01):
            ...
            fout.write(RTCTime + ","+ str(CH3)[0:6] + "\n")
...
def list_dirs():
    dirs = []
    for s in os.scandir(udisks_path):
        if s.is_dir():
            dirs.append(s)
    return dirs
...
if((chansel & 0x01) == 0x01):
    ...
    StatBut1 = Button(win, text="", padx=5, pady=1, fg="#A6ACAF", bg="#A6ACAF", activebackground="green")
    SelChan1 = Button(win, text="unused", padx=29, pady=15, fg="#A6ACAF", bg="#A6ACAF")
...
if((chansel & 0x02) == 0x02):
    ...
    StatBut2 = Button(win, text="", padx=5, pady=1, fg="black", bg="#00A0E3", activebackground="#00A0E3")
    SelChan2 = Button(win, text="unused", padx=29, pady=15, fg="black", bg="#A6ACAF")
...
if((chansel & 0x04) == 0x04):
    ...
    StatBut3 = Button(win, text="", padx=5, pady=1, fg="green", bg="green", activebackground="green")
    SelChan3 = Button(win, text="unused", padx=29, pady=15, fg="black", bg="#A6ACAF")
...
if((chansel & 0x08) == 0x08):
    ...
    StatBut4 = Button(win, text="", padx=5, pady=1, fg="#D0D1AB", bg="#D0D1AB")
    SelChan4 = Button(win, text="unused", padx=29, pady=15, fg="black", bg="#A6ACAF")
